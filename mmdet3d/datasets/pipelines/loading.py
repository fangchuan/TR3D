...
class PointSegClassMappingV2(object):
    ...
    def __init__(self, valid_cat_ids, max_cat_id=40):
        ...
        # build cat_id to class index mapping
        self.cat_id2class = -np.ones(self.max_cat_id + 1, dtype=np.int)
        for cls_idx, cat_id in enumerate(valid_cat_ids):
            self.cat_id2class[cat_id] = cls_idx
    ...
    def __call__(self, results):
        ...
        mapping = -np.ones(pts_instance_mask.max() + 1, dtype=np.int)
...
class NormalizePointsColor(object):
    ...
    def __call__(self, results):
        ...
        points = results["points"]
        assert (
            points.attribute_dims is not None and "color" in points.attribute_dims.keys()
...
class LoadPointsFromFile(object):
    ...
    def __init__(
        ...
        # print(f'Loading points from {coord_type} coordinates.')
        self.load_dim = load_dim
        self.use_dim = use_dim
        self.file_client_args = file_client_args.copy()
        self.file_client = None
    ...
    def __call__(self, results):
        ...
        # print(f'Loading points from {pts_filename} points shape: {points.shape}')

        points = points[:, self.use_dim]
        # print(f'After use_dim points shape: {points.shape}')
        attribute_dims = None
        ...
        results["points"] = points
        # print(f'After points shape: {points.shape}')
